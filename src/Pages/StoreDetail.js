...
const StoreDetail = () => {
  ...
  const getCurrentTime = async () => {
    ...
    const getTime = (await today.getHours().toString()) + (await today.getMinutes().toString());
    const getDay = await days[today.getDay()];
    ...
    <>
      <div className="detail-container">
        <div className="detail-header">
          <img className="detail-header-logo" src={storeDatas[store].logo} alt="Logo" />
          <div className="detail-header-center">
            <span className="detail-header-title">{storeDatas[store].name}</span>
        ...
        <div className="detail-sub-header">
          <div className="detail-sub-header-first">
            <div className="detail-sub-header-phone">
              ...
              <span className="detail-sub-header-phone-number detail-default-text">{storeDatas[store].phone}</span>
            </div>
            <div className="detail-sub-header-furniture">
              <div className="detail-sub-header-table">
                ...
                <span className="detail-default-text">x {storeDatas[store].tableNumber}</span>
              </div>
              <div className="detail-sub-header-chair">
                <img width="20px" src={chairIcon} alt="chair-icon"></img>
                <span className="detail-default-text">x {storeDatas[store].chairNumber}</span>
              </div>
            </div>
          </div>
          <div className="detail-sub-header-second">
            <div className="detail-sub-header-address">
              ...
              <span className="detail-sub-header-address-text detail-default-text">{storeDatas[store].address}</span>
            </div>
            <div className="detail-sub-header-hours">
              ...
              <span className="detail-sub-header-open-hours detail-default-text">
                {currentTime >= storeDatas[store].openHours[currentDay][0] && currentTime <= storeDatas[store].openHours[currentDay][1] ? "영업중" : "문닫음"}
              </span>
